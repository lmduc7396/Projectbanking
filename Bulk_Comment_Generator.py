...
import time
from utilities import quarter_to_numeric, quarter_sort_key
...
def get_bank_sector_mapping():
    ...
    # Assuming Bank_Type.xlsx has columns 'TICKER' and 'Type'
    ...
    if 'TICKER' in bank_type_mapping.columns and 'Type' in bank_type_mapping.columns:
        ...
        # If structure is different, create manual mapping based on existing logic
        banks = df_quarter[df_quarter['TICKER'].str.len() == 3]['TICKER'].unique()
        mapping = {}
        for bank in banks:
            bank_data = df_quarter[df_quarter['TICKER'] == bank]
            if not bank_data.empty:
                sector = bank_data['Type'].iloc[0]
                mapping[bank] = sector
        return mapping
...
def generate_all_comments():
    ...
    print(f"Processing {len(all_banks)} individual banks")
    ...
    all_comments = []
    total_combinations = len(all_banks) * len(quarters)
    processed = 0
    errors = 0
